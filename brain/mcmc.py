...
def estimate_pi_mcmc_chain(n_steps, step_size, burn_in):
    ...
    return (
        4 * np.sum(inside_flags[burn_in:]) / len(inside_flags[burn_in:]),
        np.array(traj_x),
        np.array(traj_y),
        np.array(inside_flags),
        np.array(accepted_flags),
        np.array(pi_running_estimate)
    )
...
for i in range(n_chains):
    pi_est, x_traj, y_traj, inside_flags, accepted_flags, running_pi = estimate_pi_mcmc_chain(n_steps, step_size, burn_in)
    ...
    print(f"Chain {i+1} π estimate: {pi_est:.6f} | Error: {error:.6f} (True π: {true_pi:.6f})")

avg_pi = np.mean(all_estimates)
end_time = time.time()
print(f"\nFinal Average Estimated π over {n_chains} chains: {avg_pi:.6f}")
print(f"Total computation time: {end_time - start_time:.2f} seconds")
# Calculate final error of average π
abs_error = abs(avg_pi - true_pi)
percentage_error = (abs_error / true_pi) * 100
...
# π Estimate Plot
ax_pi.set_xlim(0, n_steps)
ax_pi.set_ylim(true_pi - 0.5, true_pi + 0.5)
ax_pi.axhline(true_pi, color='black', linestyle='--', label='True π')
pi_lines = []
for i in range(n_chains):
...
chain_pi_x = [[] for _ in range(n_chains)]
chain_pi_y = [[] for _ in range(n_chains)]
...
def update(frame):
    ...
    rejected_scatter.set_offsets(np.column_stack((rejected_x, rejected_y)))
    ...
    avg_pi_now = np.mean([r[frame - 1] if frame < len(r) else r[-1] for r in all_running_estimates])
...
ani = animation.FuncAnimation(
    fig, update, frames=range(10, n_steps, 10),
